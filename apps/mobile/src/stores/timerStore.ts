// ---------------------------------------------------------------------------
// Mobile quick-timer store.
// Mirrors apps/desktop/src/stores/timerStore.ts but uses AsyncStorage for
// persistence and expo-haptics / ExpoNotificationService for completion.
// ---------------------------------------------------------------------------
...
async function loadPersisted(): Promise<Partial<PersistedData>> {
  try {
...
export const useTimerStore = create<TimerStore>((set, get) => {
  ...
  return {
    ...
    async hydrate() {
      ...
      let timer = stored.timer ?? INITIAL_QUICK_TIMER;
    ...
    cancel() {
      set((s) => ({
        timer: { ...INITIAL_QUICK_TIMER, durationSeconds: s.timer.durationSeconds, activePresetId: s.timer.activePresetId },
      }));
      persist();
    },
    ...
    _complete() {
      ...
      if (settings.notificationsEnabled) {
        notificationService.show({
          ...
          sound: settings.soundEnabled ? 'complete' : 'tick',
        }).catch(() => {});
      }
    ...
    reorderPresets(orderedIds) {
      ...
      const reordered = orderedIds
        ...
        .filter((p): p is TimerPreset => p !== null);
